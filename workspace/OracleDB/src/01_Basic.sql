...
--Table : 데이터 베이스에서 사용되는 데이터 집합의 단위, 흔히 알고있는 '표'방식
--Field : 열에 해당하는 데이터 단위. 속성, 애트리뷰트
--Redcord : 행에 해당하는 데이터 단위. 튜플

--위와 같이 표형식의 데이터 베이스를 "관계형 데이터 베이스"라고 부릅니다.
--현재 출시되어 데이터 베이스로 활용되는 제품 : 오라클 ,MySQL, MSSQL, MariaDB
--										MongoDB, Access등

--데이터 베이스 프로그램에는 저장하는 공간이나 기능이 있지만, 이를 가능하게 해주는 프로그램이 따로 존재
--데이터 베이스의 조작운영을 가능하게 하는 프로그램:
--										DBMS(Database Management System)
--각 데이터 베이스 제품들에는 자신의 데이터 베이스를 관리할 수 있는 DBMS가 존재한다.
--오라클 데이터 베이스에서 사용 가능한 DBKS : SQL Developer(오라클 자체 프로그램), SQLPLUS, 이클립스 등


--데이터베이스에 활용되는 언어(Language)
--SQL(Structure Query Language) : 관계형 데이터베이스 관리 시스템(RDBMS)의
