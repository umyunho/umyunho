...
-- 명령 양식
CREATE table 테이블이름(
	필드명1 DATATYPE [ DEFAULT 값 OR 제약조건],
	필드명2 DATATYPE [ DEFAULT 값 OR 제약조건],
	필드명3 DATATYPE [ DEFAULT 값 OR 제약조건],
	필드명4 DATATYPE [ DEFAULT 값 OR 제약조건],
	...
	필드명 DATATYPE [ DEFAULT 값 OR 제약조건] 
--	마지막은 ,안씀
);
...
--8. 테이블 생성시 대/소문자 구분은 하지않음(기본적으로 테이블이나 컬럼명은 대문자로 만들어짐)
--	create table
--	CREATE TABLE
--	Create Table
--9. 보통은 데이터형식과 용량(크기)를 지정하는데, DATE데이터 형식은 유형을 별도로 크기를 지정하지 않음
--10. 문자데이터의 DataType->varchar2(10), number(4)
--11. 문자 데이터 유형은 반드시 가질 수 있는 최대 길이를 표시해야함.
--12. 숫자 데이터 형식은 byte수로 표현하지 않고, 자리수로 표현 number(4) -> 4자리 숫자(0~9999)
--13. 컬럼(필드)과 컬럼(필드)의 구분은 콤마로 하되, 마지막 컬럼이나 constaint를 작성 후엔 콤마를 찍지 않음


--테이블 생성2
--테이블 이름 : MemberList(회원리스트)
--필드 : memberNum, memberName, Phone, Birth, Bpoint
--데이터 형식 : memberNum : VARCHAR2(5)
...
--외래키로 유지되는 무결성을 "참조 무결성"이라고 부른다.
--예를 들면 booklist에 존재하지 않는 도서의 번호가 rentlist의 빌려간 도서의 번호로 등록되지 못하게 하는것을 말한다.
...
CREATE TABLE rentlist(
	...
	rent_date DATE default sysdate,	--대여 날짜,sysdate 오라클에서 제공하는 오늘날짜
	--rent_seq number(3), --seq는 순번을 의미, 하느이 날짜안에서 지정된 순번
	...
	--현재 테이블의 bnum필드는 booklist테이블의 booknum필드값을 참조합니다.
	constraint fk2 foreign key(mnum) references memberlist(membernum)
	--현재 테이블의 mnum필드는 memberlist테이블의 membernuim필드값을 참조합니다.
)
SELECT * FROM rentlist;
DROP TABLE rentlist purge;
--외래키의 참조 대상이 되는 필드는 그쪽 테이블의 기본키여야한다.

--제약 조건(constraint)
--primary key
--	 테이블에 저장된 레코드를 고유하게 식별하기 위한 키, 하나의 테이블에 하나의 기본키만 정의할 수 있다.
--   여러 필드가 조합된 기본키 생성이 가능하다.
--   기본 키는 중복된 값을 가질 수 없으며 빈칸도 있을 수 없다.
-- 	PRIMARY KEY = UNIQUE KEY + NOT NULL
--UNIQUE KEY
--	테이블에 저장된 행 데이터를 고유하게 식별하기 위한 고유키를 정의한다.
-- 	단 null은 고유키 제약의 대상이 아님으로, null값을 가진 행이 여러개가 UNIQUE KEY제약에 위반하지는 않는다.
--NOT  NULL
--	비어있는 상태, 즉 아무것도 없는 상태를 허용하지 않는다. - 입력 필수
--CHECK
--	입력할 수 있는 값의 범위를 제한한다. CHECK제약으로는 TRUE or FALSE로 평가할 수 있는 논리식을 지정합니다.
--FOREIGN KEY
