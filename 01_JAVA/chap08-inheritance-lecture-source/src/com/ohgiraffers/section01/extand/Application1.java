...
public class Application1 {
    public static void main(String[] args) {
        ...
        * 부목가 가지는 메소드 선언부를 그대로 사용하면서 자식 클래스가 정의한 메소드대로 동작하도록 구현
        * 몸체 부분을 새롭게 다시 작성하는 기술이다.
        * 메소드 재정의를 하면 메소드를 호출할 시 재정의한 메소드가 우선적으로 동작하게 된다.
        * */

        /*
        * 이러한 상속이라는 기술을 사용하게 되면 얻게 되는 이점은 두가지가 있다.
        * 1. 새로운 클래스를 작성시 기존에 작성한 클래스를 재사용할 수 있다.
        *   1-1. 재사용시 생산성을 크게 향상시킬 수 있다.(새롭게 작성하는 것보다 빠르다)
        *   1-2. 공통적으로 사용하는 코드가 부모 클래스에 존재하면 수정사항이 생길 시 부모클래스만 수정해도 전체적으로 작동된다.
        *       (유지보수성 증가)
        * 2.클래스간의 계승 관계가 형성되며 다형성의 문법적인 토대가된다.
        * */

        /*
        * 상속으로 인한 단점
        * 1. 부모클래스의 기능을 추가, 변경시에 자식클래스가 정상 동작하는지 예측이 힘들다.
        *   상속 구조가 복잡해 질수록 그 영향에 대한 예측이 힘들어 이런 단점이 유지보수성이 증가한다는 장점과는 반대로
        *   유지보수에 악영향을 미친다.
        * 2. 또한 부모 클래스 변경 또한 쉽지 않다. 자식 클래스에서 중요하게 사용하는 기능인 경우 부모클래스를 변경할 시
        *   자식 클래스에 모두 영향을 줄 수 있다. 이 역시 유지보수에 악영향을 미친다.
        * 3. 부모클래스에서는 의미 있었던 기능들이 자식 클래스에서는 무의미할 수 있따.(불필요한 기능이 추가됨)
        * */

        /*
        * 장점과 단점을 고려했을 때 상속은 재사용이라는 장점만을 바라보게 되면 오용의 가능성이 있기 때문에 유지보수에 좋지 않은 코드를 작성할 확률이 높다.
        ...
        * 동일한 역량을 가지는 모든 객체는 동일한 메세지를 수신하기는 하지만 객체별로 그 메세지에 응답하는 방식이
        * 서로 다를 수 있다.
        * */

        //추상화 - 같은 기능 공통된 것을 묶어서 단순화 시키는 것.
        Car car = new Car();
        car.soundHorn();
        car.run();
        car.soundHorn();
