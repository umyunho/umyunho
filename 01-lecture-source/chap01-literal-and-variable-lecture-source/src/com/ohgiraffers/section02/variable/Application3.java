...
public class Application3 {
    public static void main(String[] args){
        ...
        * 실무적으로는 굉장히 중요하기 때문에 반즈시 숙지해야 하고
        ...
        *   1-5. 특수기호는 '-'와 '$'만 사용 가능하다ㅏ(따라서 공백은 사용할 수 없다.)
        * 2. 컴파일 에러를 발생시키지는 않지만 개발자끼리의 암묵적인 규칙
        *   2-1. 변수명의 길이 제한은 없다.(하지만 적당히)
        *   2-2. 변수명이 합성어로 이루어진 경우 첫 단어는 소문자 두번째로 시작하는 단어는 대문자로 시작한다. (camel-Case)
        *   2-3. 단어와 단어 사이의 연결은 언더스코어(_)로 사용하지 않는다.(타 언어의 네이밍 규칙이다.)
        *   2-4. 한글로 변수명을 짓는 것은 가능하지만, 권장하지는 않는다.(한글을 취급하는 다양한 방식들이 존재하기 때문에 에러를 유발시킬 수 있다.)
        ...
        *  */
        ...
        /*  목차 1-2. 예약어는 변수명으로 사용이 불가능하다.*/
        //int true = 1; //예약어는 사용이 불가능하다.

        /*  목차 1-3. 변수명은 대소문자를 구분한다.*/
        int Age = 20;   // 위에서 만든 age랑은 다른 것으로 취급한다.
        int True = 10;  // 예약어 true 와 false는 앞 문자가 소문자이므로 다른것으로 취급한다.

        /*  목차 1-4. 변수명은 숫자로 시작할 수 없다.*/
        //int 1age = 10; //숫자로 시작해서 에러발생
        int age1 = 10; //숫자가 처음에 오는 것이 아니면 섞어서 사용 가능

        /*  목차 1-5. 특수기호는 '_'과 '$'만 사용 가능하다.*/
        //int a@ge = 20; //사용 가능한 특수문자 외에는 사용이 불가능
        int _age = 20; //언더스코어는 사용가능함. 처음도 가능하고 중간이나 끝에도 가능
        int a$ge = 20; //$도 사용이 가능하다.

        /*목차2. 컴파일 에러를 발생시키지는 않지만 개발자들끼리의 암묵적인 규칙*/
        /*  목차 2-1. 변수명의 길이제한은 없다. 하지만 적당히 하자.*/
        int dsvaasdbvksadbgvjksbvksbkvbskjdbvkjsd;
        /*  목차 2-2. 변수명이 합성어로 이루어진 경우 첫 단어는 소문자 두번째 시작 단어는 대문자로 시작한다.*/
        /*           자바에서는 클래스 명만 유일하게 대문자로 시작한다.(camel표기법 / 낙타 표기법)*/
        int maxAge = 20;
        int minAgeFem = 20;
        ...
        String s;
        String name; //문자열 형태의 이름이 저장되겠구나라는 의도 파악이 가능함

        /*  목차 2-6. 전형적인 변수 이름이 있다면 가급ㅂ적 사용하도록 한다.*/
        int sum = 0;
        int max = 10;
        int min = 0;
        int count = 1;
